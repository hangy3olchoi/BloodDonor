...
conn = sqlite3.connect("systemServer.db", isolation_level = None) # 확장자를 반드시 .db로 줘야함, 'isolation_level = None' 생략 가능
...
cursor.execute('''
    ...
    (id text primary key,
     ...
     bloodType text,
     remark text)
''') # 't1'이라는 테이블이 존재 하지 않는다면 '항목 타입 프라이머리키(해당될 경우),' 의 형식으로 입력 
